...
- **初始排程**:`rec(wᵢ)@3i, id(wᵢ)@3i+4, spell(wᵢ)@3i+8` 排序 — 每字的階段間隔約 2–3 個其他任務。挖不出空格又只剩單一 tile 的字**不排拼字**(一格拼圖是送分題);判斷由 `SpellForm` 統一,排程閘門與板子讀同一條規則。
...
- **拼字**:兩種板子,由 `SpellForm` 依字形分流,操作手感相同(打散的選項池、由左至右填、點已填的格取回、填滿自動判定、錯了同樣 peek + requeue + 重新打散)。
  - **挖空拼字(SpellGapView)— 英文**:整個字照常顯示,挖掉其中 1–3 塊讓使用者從一池相似字塊補回(`pres ___ vative`,選 ur / ar / or / er / ir)。從零重排整串考的是「記不記得每個字母」,而英文拼字真正會錯的只有幾個位置 —— 模糊母音、母音組、字尾家族、雙子音,`SpellGaps` 挖的就是那些。格數依字長:≤5 字母 1 格、6–9 兩格、≥10 三格,挖不到那麼多就自動降。挖空位置不隨重試改變(答錯的那塊才是要再考的),只有選項順序重新打散;空格寬度一律取池中最長選項,不隨答案長度伸縮以免洩漏字數。答錯時每一格各自標紅標綠,看得出是哪一塊錯。
  - **拼字塊(TilesView)— 日文讀音**:只給圖+中文(不給字),把打散的 tile 拼回。假名讀音沒有可挖的拼寫易錯點,所以仍考整串。棋盤規則見 `NewFlowTasks.swift`:每個空白 token 一列(空白不是 tile)、小假名(ゃゅょ等)黏到前一 mora、日文以 `reading`(假名)出題、總 tile 上限 10(超過就 re-chunk);scramble 依 (item, attempt) 決定性生成,且保證不會直接排成答案。
...
每題:`answer`(4 選 1 MCQ)。**看圖選字選錯不會結束這一題**:該選項套上 3pt 紅框、
停止回應,其餘選項照常可點,題目留著讓使用者自己找到答案;只有選到對的那一次才結算,
而且只要中途排除過任何一個,那一次就以「答錯」結算。(聽句是兩張圖二選一,排除一張等於拿到
答案,所以它第一次點擊就結算。)選錯的那一列會**晃動一下**——框是狀態、晃動是事件。
結算後三種路徑,而且**揭示表晚 600ms(`revealDelay`)才升起**,讓墨塊與框先被看到:

| 情境 | 行為 |
|---|---|
| 一次就中且快(建議評分 ≠ 困難) | **自動套用建議評分**,閃現膠囊(700ms)直接下一題,不彈 sheet |
| 一次就中但慢 | Reveal sheet,手動選 困難/穩定/熟練 |
| 中途選錯過 | Reveal sheet,只有 重來/困難(困難=「按錯了其實記得」;更高評分會讓漏掉的字跳過重學) |
